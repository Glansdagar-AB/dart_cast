...
void main() {
  ...
  group('DLNA integration', () {
    test(
      ...
      expect(
        server.capturedActions.any((a) => a.action == 'Seek'),
        isTrue,
      );
      ...
      expect(
        server.capturedActions.any((a) => a.action == 'Pause'),
        isTrue,
      );
      ...
      expect(
        server.capturedActions.any((a) => a.action == 'Stop'),
        isTrue,
      );
    ...
    test('loadMedia sends SetAVTransportURI with correct URL and title',
      ...
      final setUri = server.capturedActions
          .firstWhere((a) => a.action == 'SetAVTransportURI');
      expect(setUri.body, contains('example.com/test.m3u8'));
    ...
    test('stop transitions to idle and stops polling', () async {
      ...
      final pollActions = server.capturedActions
    ...
    test('position polling reports position and duration from server',
      ...
      final pos = await session.positionStream.first
          .timeout(const Duration(seconds: 5));
      final dur = await session.durationStream.first
          .timeout(const Duration(seconds: 5));
